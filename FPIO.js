Game.registerMod("fthof_planner_internal", {
    init: function() {
        ...
        function updateFtHoFPlannerCache() {
            ...
            for (let i = 1; i <= 10; i++) {
                ...
                let normalRng = new Math.seedrandom(trueSeed + futureCast);
                let seasonRng = new Math.seedrandom(trueSeed + futureCast);
                let normalFailRng = new Math.seedrandom(trueSeed + futureCast);
                let seasonFailRng = new Math.seedrandom(trueSeed + futureCast);

                let localRngForSeed = new Math.seedrandom(trueSeed + futureCast);
                localRngForSeed();
                let rawSeedValue = localRngForSeed();

                let normalSuccess = Game.mods['fthof_planner_internal'].predictRawFtHoF(0, 0, normalRng);
                let seasonSuccess = Game.mods['fthof_planner_internal'].predictRawFtHoF(0, 1, seasonRng);
                let normalFail = Game.mods['fthof_planner_internal'].predictRawFtHoF(1, 0, normalFailRng);
                let seasonFail = Game.mods['fthof_planner_internal'].predictRawFtHoF(1, 1, seasonFailRng);
                
                let failCondition = Game.mods['fthof_planner_internal'].getRawFailCondition(rawSeedValue);
    ...
    predictRawFtHoF: function(backfire, isSeasonMod, localRng) {
        localRng();
        let choice = '';
    ...
    getRawFailCondition: function(rawSeedValue) {
    ...
    renderFtHoFPlanner: function() {
        if (Game.onMenu != 'prefs') return;

        let menu = document.getElementById('menu');
        if (!menu) return;

        this.updateFtHoFPlannerCache();
        ...
        let html = `
            <div style="text-align: center; margin-bottom: 10px;">
                <h3 style="color: #ecc45e; font-size: 18px; margin: 0;">FtHoF プランナー (v56.0.0)</h3>
